...
void AMyFPCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	InputComponent->BindAction("Pickup", IE_Pressed, this, &AMyFPCharacter::Interact);
}
...
void AMyFPCharacter::Interact()
{
	...
	for (int i = 0; i < inRangeItems.Num(); i++)
	{
		...
		if (testItem && !testItem->IsPendingKill() && testItem->GetActive())
		{
			...
			if (wielded && gunUp)
			{
				wielded->GetSkeletalMeshComponent()->SetAnimation(gunUp);
				wielded->GetSkeletalMeshComponent()->PlayAnimation(gunUp, false);
				wielded->SetActorHiddenInGame(false);
			}
			if (grabGun)
			{
				GetMesh()->SetAnimation(grabGun);
				GetMesh()->PlayAnimation(grabGun, false);
			}
		}
	}
}
